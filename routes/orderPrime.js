...
router.get('/', function (req, res) {
    ...
    const s3Key = `orderPrime-${query}`;
    const redisKey = `orderPrime:${query}`
    ...
    return redisClient.get(redisKey, (err, result) => {
      if (result) {
        ...
        return res.render('orderPrime',{ ...resultJSON, query: query, source: 'Redis Cache' });
      } else {
        console.log('nothing in cache, checking s3')
  
        // Check S3
        const params = { Bucket: app.bucketName, Key: s3Key };
        ...
        return new AWS.S3({ apiVersion: '2006-03-01' }).getObject(params, (err, result) => {
          if (result) {
            ...
            return res.render('orderPrime',{ query: query, response: state , source: 's3 storage'});
          } else {
            // Perform calculation
            console.log('nothing in s3 performing calculation')
            state = maths.orderPrime(query)
            ...
            return res.render('orderPrime',{ query: query, response: state , source: 'server calculation'});
          }
        });
      }
    });
